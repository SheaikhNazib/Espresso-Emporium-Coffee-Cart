...
export default function View() {
    ...
    return (
        <div className="max-w-6xl mx-auto p-20 bg-white shadow-md rounded-md mt-10" style={{ backgroundColor: '#f4f3f0' }}>
            ...
            <img src={coffee.photo} alt={coffee.name} className="w-full h-64 object-contain rounded-md" />
            <h2 className="text-3xl font-bold mt-4 text-center">{coffee.name}</h2>
            <div>
            ...
            <Link to={`/updateCoffee/${id}`}>
                <button className="btn w-1/6 mt-3 font-thin btn-hover hover:text-white" style={{ backgroundColor: '#d2b48c', fontFamily: 'Sour Gummy, sans-serif', textShadow: '2px 2px 4px rgba(0, 0, 0, 0.5)' }}>Update Coffee</button>
            </Link>
        </div>
    );
}